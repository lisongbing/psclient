...
export default class ListView extends cc.Component {
    ...
    public notifyUpdate(updateIndex?: number[]) {
        ...
        cc.log('this._itemWidth-->' + this._itemWidth)
        cc.log('this.this.spacing-->' + this.spacing)

        if (this.horizontal) {
            this.getContent().width = this.adapter.getCount() * (this._itemWidth + this.spacing) + this.spacing;
        } else {
            this.getContent().height = this.adapter.getCount() * (this._itemHeight + this.spacing) + this.spacing; // get total content height
        }
