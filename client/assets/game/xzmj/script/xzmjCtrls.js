...
let MajhHandCardView = cc.Class({
    ...
    onTouchstart: function (event) {
        ...
        if (parseInt(tg.code) == 50 || tg.zezao || this.pPage.isAutoHu) {
            return
        }
    ...
    crtPlayerItem: function (vbPlayerContent, allPerResultItem) {
        ...
        if (!cc.g.utils.judgeArrayEmpty(mingtang)) {
            ...
            for (let i = 0; i < mingtang.length; i++) {
                ...
                let spr = new cc.Node();
                spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('mingtang_xz_' + mingtang[i]);
