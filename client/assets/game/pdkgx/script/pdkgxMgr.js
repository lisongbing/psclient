...
let Logic = require('pdkgxLogic');
let Audio = require('pdkgxAudio');
let Player = require('pdkgxPlayer');
let MsgQueue = require('pdkgxMsgQueue');
let BackPlay = require('pdkgxBackPlay');
...
cc.Class({
    ...
    setNetworkMessageCallback: function () {
        ...
        // 结算结果通知
        cc.g.networkMgr.addHandler(PB.PROTO.NOTIFY_GX_PDK_PLAYER_RESULT_LIST, (resp) => {
    ...
    playerMoneyUpdated:function (player, coin) {
        cc.log('playerMoneyUpdated', player, coin);
    ...
    prepareSettleData: function (resp) {
        {/*
            //@api:1030,@type:resp
            //@api:1043,@type:resp
            message GongXianPdkPlayerResultListNtf{
                int64   winUid = 1;//赢得的玩家
                int64   base    = 2;//底分
                int32   round    = 3;//当前局数
                int64   time    = 4;//时间戳
                repeated GongXianPdkResultInfo list = 5;  //玩家牌的信息及输赢
            }
            //跑的快的结算结果
            message GongXianPdkResultInfo{
                int64   uid = 1;//玩家ID
                int64   winlose = 2;//胜利失败
                int32   bombnum = 3;//炸弹数
                int64   totalscore = 4;//积分
                int64   bombscore = 5;//炸弹积分
                repeated int32 deskcards = 6;//已打出的牌ID
                repeated int32 handcards =7;//手牌ID
                bool    isBaoPei=8;//是否包赔
                bool    isChengFive =9;//是否报单乘5
            }
        */}
        ...
        resp.list.forEach(e => {
            ...
            sd.isBaoPei = e.isBaoPei;
            sd.isChengFive = e.isChengFive;
