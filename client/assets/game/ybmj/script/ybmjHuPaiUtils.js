...
var HuPaiUtils = {
	...
	benjing: 0,
	guiValues: [],
	setRules: function(rules,benjing){
		this.benjing = benjing;
		for (var i in rules){
			if (rules[i] == 16){
				this.mingtangToFun["无听用"] = 3;
			}
			if (rules[i] ==12){  //四听用
				if (benjing % 10 == 9){
					this.guiValues = [benjing-8];
				}else{
					this.guiValues = [benjing+1];
				}
			}else if (rules[i] ==13){ //8听用
				if (benjing % 10 == 9){
					this.guiValues = [benjing-1,benjing-8];
				}else if  (benjing % 10 == 1){
					this.guiValues = [benjing+8,benjing+1];
				}else{
					this.guiValues = [benjing-1,benjing+1];
				}
			}else if (rules[i] ==14){//11听用
				if (benjing % 10 == 9){
					this.guiValues = [benjing-1,benjing,benjing-8];
				}else if  (benjing % 10 == 1){
					this.guiValues = [benjing+8,benjing,benjing+1];
				}else{
					this.guiValues = [benjing-1,benjing,benjing+1];
				}
			}
			//无听用
			if (rules[i] ==16){
    ...
    //传入手牌,及缺的门数,规则
    showHuPaiPrompt:function(handleArr,putoutCards,que,rules,benjing){
        let result = new Map();
		this.setRules(rules,benjing);
        this.back = [];
        this.result = [];
		////cc.log(rules);
    ...
    computQiduiFun:function(cards,putOutCards,hucard,result){
		...
		for (var i=0;i<four.length;i++){
			...
			//本金  接本金要减少1归，不算了
			if (this.benjing == four[i]){
				fun += this.mingtangToFun["本金"];
			}
		}
    ...
    computeGui:function(cards,putOutCards){
        ...
        for (var i=0;i < cards.length;i++) {
			if (cards[i].length == 2){
				...
				if (cards[i][0] == cards[i][1]){
					...
					if (cards[i][0]==this.benjing && benjing == 0){
						benjing = 1;
						gui++;
					}
        ...
        for (var i=0;i<putOutCards.length;i++){
            if (putOutCards[i].length == 3){
				...
				if (putOutCards[i][0]==this.benjing && benjing == 0){
						benjing = 1;
						gui++;
				}
            }
		...
		if (three >=3 && beijing==0){
			three -=3;
			gui++;
			beijing =1;
		}
		//接本金
		if  (three > 0&&benjing > 0){
			if (gui > 0){
				gui--
			}
		}
