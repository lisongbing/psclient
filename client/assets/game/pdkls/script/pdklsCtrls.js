...
let LocSettleView = cc.Class({
    ...
    show: function () {
        ...
        // 地区
        this.Label_diqu.string = '乐山跑得快'; //cc.g.areaInfo[ri.origin].name + '跑得快';
        // 房间号
...
let LocPlayerView = cc.Class({
    ...
    upOutCards: function () {
        ...
        if (this.player.outType==DEF.ComType.FJ) {
            PG.comPubAnm('feij');
        } else if (this.player.outType==DEF.ComType.ZD || this.player.outType==DEF.ComType.XZ) {
            PG.comPubAnm('zhadan');
