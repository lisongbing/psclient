...
var HuPaiUtils = require("lzmjHuPaiUtils");
var lzmjCtrls = require('lzmjCtrls');
...
cc.Class({
    ...
    resetDatas() {
        ...
        this.clickGanged = false

        // 最多3张鬼牌
        WangCodeUtils.guiCode1 = 0;
        WangCodeUtils.guiCode2 = 0;
        WangCodeUtils.guiCode3 = 0;
    ...
    initPaiCount: function() {
        ...
        let showCount = allCount - 14 - (palerCount - 1) * 13 + hongZhongCount
    ...
    getCanPlayHomgZhongRule: function() {
        // 18 开局定缺 19 胡牌缺一门 20 两放麻将
        let canHuan = false;
        let newRlue = this.gameMgr.roomInfo.NewRlue
        //  找红中个数
    ...
    initPlayerView: function () {
        ...
        while (true) {
            ...
            let view = new lzmjCtrls.PlayerView();
    ...
    upPage: function () {
        ...
        let gangObj = cc.g.utils.getLocalStorage('clickGanged')
        if (!cc.g.utils.judgeObjectEmpty(gangObj)) {
            this.clickGanged = true;

            if (this.getCanPlayHomgZhongRule() && this.clickGanged) {
                const palyerViewItem = this.playerView[0];
                let handlePaiArray = palyerViewItem.handCardView.hcGroups;
                handlePaiArray.forEach(org => {
                    if (org.active && org.code == 50) {
    ...
    beginSendCard: function () {
        // // 每局完了，清除
        // cc.g.utils.clearLocalStorage('GANG_ARR')
    ...
    onButtonCopyRoom: function (event, customEventData) {
       ...
       if (ri.clubId) {
        ...
        // des += cc.g.areaInfo[ri.origin].name + '泸州麻将,';
        des += '泸州麻将,';
        des += ri.GameNum + '局,';
        des += ri.total + '人';
    ...
    onButtonInvite: function (event, customEventData) {
        ...
        let name = gameTypes[ri.gameType] ? gameTypes[ri.gameType].NAME : '泸州麻将';
    ...
    doRecivePiao: function (palyerViewItem, status) {
        ...
        //
        // // 显示
        // palyerViewItem.Sprite_Header_Piao_Bg.active = true;
        // palyerViewItem.Sprite_Header_Play_Vaule.active = true;
        // palyerViewItem.piao = status
        // if (status > 0) {
        //     palyerViewItem.Sprite_Header_Piao_Bg.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_piao_small_bg');
        //     palyerViewItem.Sprite_Header_Play_Vaule.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_piao_small_'+status);
        //     // palyerViewItem.Sprite_Piao_Img.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('piao');
        // } else {
        //     palyerViewItem.Sprite_Header_Piao_Bg.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_bupiao_small');
        //     palyerViewItem.Sprite_Header_Play_Vaule.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_piao_small_'+status);
        // }
    },
    ...
    doReciveReConnectPiao: function (palyerViewItem, status) {
        ...
        // } else {
        //     palyerViewItem.Sprite_Header_Piao_Bg.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_bupiao_small');
        //     palyerViewItem.Sprite_Header_Piao_Bg.active = false;
        //     // palyerViewItem.Sprite_Header_Play_Vaule.getComponent(cc.Sprite).spriteFrame = this.majhAtlas0.getSpriteFrame('ny_piao_small_'+status);
        // }
    },
    // 收到服务器发牌消息
    ...
    onClickQiPaiBtnClicked: function(customData) {
        ...
        if (player.d.status == LG_Sta.CanHuan3.v) { // 换3张操作
            ...
            // 红中，则返回 或者 显示操作按钮时候 返回
            if (qiPaiItemTwo.zezao || qiPaiItemTwo.yellowZe) {
                return
            }
            ...
            // 王牌可出
            let canOutGui = WangCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);
            if (!canOutGui) {
                // 红中，则返回 或者 显示操作按钮时候 返回
                if (qiPaiItemTwo.code == 50 || this.Node_HuTiGangView.active || qiPaiItemTwo.zezao || qiPaiItemTwo.yellowZe) {
                    return
    ...
    doAutoDingQueAlert: function (palyerViewItem) {
        if (palyerViewItem.index == 0) {
            ...
            for (let i = 0; i < handlePaiArray.length; i++) {
                let org = handlePaiArray[i]
                if (org.active) {
                    if ((org.code >= 1 && org.code<=9) && ((org.code != WangCodeUtils.guiCode1) &&
                        (org.code != WangCodeUtils.guiCode2) && (org.code != WangCodeUtils.guiCode3))) {
                        x++;
                    } else if ((org.code >= 11 && org.code<=19) && ((org.code != WangCodeUtils.guiCode1) &&
                        (org.code != WangCodeUtils.guiCode2) && (org.code != WangCodeUtils.guiCode3))) {
                        y++;
                    } else if ((org.code >= 21 && org.code<=29) && ((org.code != WangCodeUtils.guiCode1) &&
                        (org.code != WangCodeUtils.guiCode2) && (org.code != WangCodeUtils.guiCode3))) {
                        c++;
                    }
                }
    ...
    doPlayMj: function (palyerViewItem, v) {
        cc.log('打牌打牌打牌打牌打牌打牌打牌打牌打牌打牌打牌打牌打牌-->' + v)
        // 记录是否包含杠操作,
        // if (this.Button_Gang.active) {
        //     // 获取可以杠的牌
        //     let gangArr = this.doCheckGang();
        //     let saveGangArr = cc.g.utils.getLocalStorage('GANG_ARR')
        //     if (cc.g.utils.judgeArrayEmpty(saveGangArr)) {
        //         saveGangArr = []
        //     }
        //
        //     if (!cc.g.utils.judgeArrayEmpty(gangArr)) {
        //         gangArr.forEach((code)=>{
        //             saveGangArr.push(code)
        //         })
        //         // 保存数据
        //         cc.g.utils.setLocalStorage('GANG_ARR', saveGangArr)
        //     }
        // }
        ...
        if (this.getCanPlayHomgZhongRule() && this.clickGanged) {
            ...
            cc.g.utils.clearLocalStorage('clickGanged')
        }
    },
    ...
    upOperate: function () {
        ...
        // let isReConnect = this.playerView[0].player.isReConnect
        // // 断线重连回来不处理,以服务器为准
        // if (isReConnect) {
        //     //cc.log('断线重连。。。不用更新按钮..');
        // } else {
        //     // 检查杠碰牌
        //     let gangPongArr = this.doCheckHandleCardPengGang();
        //     let isCanGang = (gangPongArr[0] == 1) ? true : false
        //     let isCanPong = (gangPongArr[1] == 1) ? true : false
        //
        //     // 没有碰杠牌
        //     if (isCanGang && isCanPong) {
        //         btn[2].active = true;
        //         btn[4].active = true;
        //     } else if (!isCanGang && isCanPong) {
        //         btn[2].active = false;
        //         btn[4].active = true;
        //     } else if (isCanGang && !isCanPong) {
        //         btn[2].active = true;
    ...
    upDateOperateByUser: function () {
        ...
        if (isCanGang && isCanPong) {
            ...
            btn[4].active = true;
            btn[5].active = true;
            this.Node_HuTiGangView.active = true
        } else if (!isCanGang && isCanPong) {
            btn[2].active = false;
            btn[4].active = true;
            btn[5].active = true;
    ...
    doCheckHandleCardPengGang: function() {
        // 数组第 0个 杠 0/1
        // 数组第  1个 碰 0/1
        let retArr = [0, 0];
        let handlePaiArray = this.playerView[0].handCardView.hcGroups;
        ...
        // 计算手牌是否有碰杠操作
        if (WangCodeUtils.canHongPlay) { // 红中参与计算
            let hongLen = hongArr.length
            if (hongLen > 0) {
                if (hongLen >= 3) {
                    if (this.doCheckCanPlayDaPai()) { //  可以打牌, 只判断手牌
                        retArr[0] = 1
                        // retArr[1] = 1
                    } else {
                        for (let i = 0; i < newHandArr.length; i++) {
                            if (newHandArr[i].code == this.codeLight) {
                                retArr[0] = 1
                                retArr[1] = 1
                            }
                        }
                    ...
                    if (this.doCheckCanPlayDaPai()) {
                        for (let i = 0; i < resArr.length; i++) {
                            ...
                            if (recountNum == 4) {
                                ...
                                let cardObj = resArr[i][0]
                                if (cardObj.code != 50) { // 排除红中
                                    if (cardObj.code == this.codeLight) {
                                        retArr[0] = 1
                                    }
                                }
                            } else if (recountNum == 2) {
                                let cardObj = resArr[i][0]
                                if (cardObj.code != 50) { // 排除红中
                                    if (cardObj.code == this.codeLight) {
                                        retArr[0] = 1
    ...
    showSelectTiView: function(arr) {
        ...
        arr.forEach((code)=>{
            ...
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'lzmj', 'onClickTiClicked', cardNode);
            // nodejs
    ...
    showSelectGangView: function(arr) {
        ...
        arr.forEach((code)=>{
            ...
            lastOneX = viewIndex *  DEF.hcMultHuTiPos[0].moveBy.z + (viewIndex + 1) * (DEF.hcMultHuTiPos[0].moveBy.z/2)
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'lzmj', 'onClickGangClicked', cardNode);
            // nodejs
            this.huTi_Node_Layout.addChild(cardNode, 1, 'Node_card_Num'+viewIndex);
            // get card node
            itemArr.push(cardNode)
        ...
        if (hzNum > 0) {
            ...
            if (hzNum >= 3) {
                if (this.doCheckCanPlayDaPai()) {
                    ...
                    if (!cc.g.utils.judgeArrayEmpty(pongPaiArr)) {
                        pongPaiArr.forEach((pong)=> {
                            if ((pong.type == 'peng') || (pong.type == 'fei')) {
                                pongArr.push(pong)
                                newCodeArr.push(pong.code)
                            }
                        })
                    }
                    ...
                    //  去重复
                    let allGangArr=[]
                    if (!cc.g.utils.judgeArrayEmpty(newCodeArr)) {
                        for(let i=0;i<newCodeArr.length;i++){
                            if(allGangArr.indexOf(newCodeArr[i]) == -1){
                                allGangArr.push(newCodeArr[i])
                            }
                        }
                    }

                    // //cc.log('allGangArr num> 3', allGangArr)
                    ...
                    // //cc.log('newAllGang num> 3', newAllGang)
                ...
                if (this.doCheckCanPlayDaPai()) {
                    ...
                    if (canGangCount > 1) {
                        ...
                        let newCodeGangArr = []
                        if (!cc.g.utils.judgeArrayEmpty(allGangArr)) {
                            for(let i=0;i<allGangArr.length;i++){
                                if(newCodeGangArr.indexOf(allGangArr[i]) == -1){
                                    newCodeGangArr.push(allGangArr[i])
                                }
                            }
                        }

                        // //cc.log('newCodeGangArr=num=2>', newCodeGangArr)
                        ...
                        if (!cc.g.utils.judgeArrayEmpty(newCodeGangArr)) {
                            newCodeGangArr.forEach((code)=>{
                                if ((tong == 1) && (tiao == 1)) {
                                    if (code > 0 && code < 10) {
                                        newAllGang.push(code)
                                    } else if (code > 10 && code < 20) {
                            ...
                            if (cc.g.utils.judgeArrayEmpty(newAllGang)) {
                                newAllGang = newCodeGangArr
                            }
                        }

                        // //cc.log('newAllGang=num=2=>', newAllGang)
                        ...
                        // 杠牌
                        // this.showSelectGangView(newCodeGangArr)
                    } else {
                        // 优先处理别人打的杠牌
                        if (!isAnCanGang) {
                            this.gameMgr.sendOp(DEF.PlayerOpt.Gang.v, this.codeLight);
    ...
    doCheckGang: function () {
        ...
        for (let i = 0; i < resArr.length; i++) {
            ...
            if (recountNum == 4) {
                let cardObj = resArr[i][0]
                // (cardObj.code != 50) &&
                if (((cardObj.code != WangCodeUtils.guiCode1)
                    && (cardObj.code != WangCodeUtils.guiCode2)
                    && (cardObj.code != WangCodeUtils.guiCode3))) { // 排除红中
                    repeatArr.push(cardObj)
                }
            } else if (recountNum == 3) {
                let cardObj = resArr[i][0]
                ...
                if (((cardObj.code != WangCodeUtils.guiCode1)
                    ...
                    if (cardObj.code == this.codeLight) { // 3个的
                        isAnCanGang = false
                        // gangArr.push(parseInt(cardObj.code))
                    }
                }
            }
        }
        ...
        if (!isAnCanGang) {
            ...
            if (canGangCount > 1) {
                ...
                //     repeatArr.forEach((anItem)=>{
                //         allGangArr.push(parseInt(anItem.code))
                //         // gangArr.push(parseInt(anItem.code))
                //     })
                // }
                ...
                if (!cc.g.utils.judgeArrayEmpty(pongArr)) {
                    pongArr.forEach((pItem)=>{
                        // allGangArr.push(parseInt(pItem.code))
                        gangArr.push(parseInt(pItem.code))
                    })
                }
            } else {
                if (!cc.g.utils.judgeArrayEmpty(repeatArr) && isAnCanGang) { // 暗杠 服务器不推消息，自己遍历发送消息
                    // gangArr.push([repeatObj.code])
                } else if (!cc.g.utils.judgeArrayEmpty(pongArr)) {
                    let pongObj = pongArr[0]
                    gangArr.push([pongObj.code])
                } else { // 明杠
                    gangArr.push(canOptVal)
                }
            }
        }
        ...
        if (canOptType == 1) {
            ...
            if (deskId == 0 || this.isbpm) {
                ...
                for (let i = 1; i < val.length; i++) {
                    ...
                    //         (org.code == gang.code1 && org.active) ||
                    //         (org.code == gang.code2 && org.active) ||
                    //         (org.code == gang.code3 && org.active)) {
                    //         if (curIndex < 4) {
                    //             org.active = false
                    //             curIndex++;
                    //         }
            ...
            if (deskId == 0 || this.isbpm) {
                ...
                //     }
                // });
                // realCode.forEach((rCode)=>{
                //     // 碰牌后，删除数组中碰的数据
                //     handlePaiArray.forEach(org => {
                //         if ((org.code == rCode) && org.active) {
                //             org.active = false
                //         }
                //     });
                // })
    ...
    doReciveGang: function (palyerViewItem, v) {
        ...
        if (getRelDeskId == 0 && !this.isbpm) {
            ...
            this.clickGanged = true;
            let gangObj = {}
            gangObj.clickGanged = true;
            cc.g.utils.setLocalStorage('clickGanged', gangObj)

            // // 检测是否有杠后不能打红中规则
            // if (this.getCanPlayHomgZhongRule()) {
            //     let handlePaiArray = palyerViewItem.handCardView.hcGroups;
            //     handlePaiArray.forEach(org => {
            //         if (org.active && org.code == 50) {
            //             let Sprite_ZeZao = cc.find("Sprite_ZeZao", org)
    ...
    onGameSettlement: function () {
        if (! this.settleView) {
            this.settleView = new lzmjCtrls.SettleView();
    ...
    onGameSettleFinal: function () {
        if (! this.settleFinalView) {
            this.settleFinalView = new lzmjCtrls.SettleFinalView();
    ...
    clearWaitCard: function () {
        ...
        for (let i = 0; i < this.playerView.length; i++) {
            ...
            // 隐藏票
            palyerViewItem.Sprite_Header_Piao_Bg.active = false;
            palyerViewItem.Sprite_Header_Play_Vaule.active = false;
    ...
    showZeZaoImg: function(palyerViewItem) {
        if (palyerViewItem.index == 0) {
            ...
            handlePaiArray.forEach((org)=>{
                ...
                let Sprite_topMeng = cc.find("Sprite_Wang", org)
            ...
            handlePaiArrayActive.forEach(org => {
                ...
                if (queIndex == 0) {
                    if (org.code >= 1 && org.code<=9) {
                        let findGui = false;
                        // if (org.code == WangCodeUtils.guiCode1) {
                        //     findGui = true;
                        // }
                        //
                        // if (org.code == WangCodeUtils.guiCode2) {
                        //     findGui = true;
                        // }
                        //
                        // if (org.code == WangCodeUtils.guiCode3) {
                        //     findGui = true;
                        // }
            ...
            if (haveQuePai) {
                ...
                //     // 听用
                //     let tingIndex = WangCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);
                //
                //     // 显示黄色
    ...
    doReciveHu: function (palyerViewItem, v) {
        ...
        if (isQianGang && palyerViewItemUid != null) {
            ...
            for (let i = 0; i < pongPaiArray.length; i++) {
                ...
                if (pongItem.code == parseInt(v[1])) { // 找到杠牌了
                    pongPaiArray.splice(i, 1);
                    nodeCodeArr.push(pongItem.code)
                    nodeCodeArr.push(pongItem.code1)
                    nodeCodeArr.push(pongItem.code2)
                    nodeCodeArr.push(pongItem.code3)
                    break;
                }
            }
    ...
    //     return tingCodeArr;
    // },
    doShowTingAnmi: function(showAnima, code) {
        ...
        if (showAnima) {
            ...
            self.scheduleOnce(()=>{
                ...
                self.Node_Ting_Anmi.setScale(1.2, 1.2)
                self.Node_Ting_Anmi.runAction(
                    cc.sequence(
                        cc.delayTime(0.2),
                        cc.spawn(
    ...
    doRealReciveBenJing: function(palyerViewItem, code, showAnima) {
        ...
        //     this.benjing = code
        //     let tingIndex = WangCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);
        //     let tingCodeArr = WangCodeUtils.doGetTingCodeArr(tingIndex, code);
        ...
        //     WangCodeUtils.doGetTingCode(this.tingCodeArr, this.gameMgr.roomInfo.NewRlue);
        // }
    },
    doReciveBenJing: function(palyerViewItem, code, showAnima) {
        ...
        if (palyerViewItem.index == 0) {
            let tingIndex = 0
            this.benjing = code
            /*let canPlayWang = WangCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);*/
            let tingCodeArr = WangCodeUtils.doGetTingCodeArr(tingIndex, code);
            this.tingCodeArr = tingCodeArr
    ...
    doShowZhuMaView: function(maArr, callback) {
        const self = this

        //  显示抓马
        self.doRealReciveZhuaMa(maArr, true)
        // 5s 后返回
        this.scheduleOnce(() =>{
            self.Node_Zhua_Ma.active = false;
            // 返回
            callback()
        }, 4);
    },
    getMaVauleNum: function(maArr) {
        let num = 10
        maArr.forEach((code)=>{
            if ((parseInt(code) == 1) || (parseInt(code) == 11) || (parseInt(code) == 21)) {
                num = 10
            } else {
                num = parseInt(code)
            }
    ...
    getOtherMaVauleNum: function(code) {
        // 是否是马值
        let oMaVaule = false
        let numCode = parseInt(code)
        if (((numCode % 10) == 1) || ((numCode % 10) == 5) || ((numCode % 10) == 9)) {
            oMaVaule = true
        }

        // 王牌和抓马都算
        if ((numCode == WangCodeUtils.guiCode1) || (numCode == WangCodeUtils.guiCode2) || (numCode == WangCodeUtils.guiCode3)) {
            oMaVaule = true
        }

        return oMaVaule
    },
    doShowAnimaMa: function(vIndex, allMaVaule, nodeArr, codeList, mIndex) {
        const self = this
        let nodeArrLen = nodeArr.length;
        let cardNode = nodeArr[vIndex]
        cardNode.active = true
        // 初始化位置
        let pox = DEF.hcMaPos[0].moveBy.x + vIndex * DEF.hcMaPos[0].moveTo.x
        self.scheduleOnce(()=>{
            cardNode.runAction(
                cc.sequence(
                    cc.delayTime(0.15),
                    cc.spawn(
                        cc.moveTo(0.1, pox, DEF.hcMaPos[0].moveBy.y),
                        cc.fadeTo(0.1, 255),
                    ),
                    cc.callFunc(
                        function (params) {
                            vIndex ++
                            if (allMaVaule) {
                                let num = self.getMaVauleNum(codeList)
                                self.Node_Ting_Layout_Label.string = 'X ' + num
                            } else {
                                let oMaVaule = self.getOtherMaVauleNum(cardNode.code)
                                if (oMaVaule) {
                                    mIndex++
                                    self.Node_Ting_Layout_Label.string = 'X ' + mIndex
                                }
                            }

                            if (vIndex < nodeArrLen) {
                                self.doShowAnimaMa(vIndex, allMaVaule, nodeArr, codeList, mIndex)
                            }
                        },
                        self,null
                    )
    ...
    doRealReciveZhuaMa: function(codeList, showAnima) {
        const self = this

        this.Node_Zhua_Ma.active = true;
        this.Node_Ting_Layout_Label.string = 'X 0'
        ...
        codeList.forEach((code)=>{
            ...
            cardNode.setPosition(DEF.hcMaPos[0].moveStart.x, DEF.hcMaPos[0].moveBy.y)

            // add
            self.Node_Ting_Layout.addChild(cardNode);

            nodeArr.push(cardNode);
            // // 初始化位置
            // let pox = DEF.hcMaPos[0].moveBy.x + vIndex * DEF.hcMaPos[0].moveTo.x
            //
            // self.scheduleOnce(()=>{
            ...
            //         cc.sequence(
            //             cc.delayTime(0.2),
            //             cc.moveTo(0.2, pox, DEF.hcMaPos[0].moveBy.y),
            //         )
            //     )
            //     // cardNode.active = true
            // }, 0.2);
        ...
        //
        // rule 4
        let maVaule = this.getAllMaVaule();

        this.doShowAnimaMa(0, maVaule, nodeArr, codeList, 0)
        //
        // // 文字label
        // this.Node_Ting_Layout_Label = cc.find("Node_ZhuaMa/zhuaMaAnim/Node_Pai/Label_Bei", r).getComponent(cc.Label);
        //
    ...
    doGetPassHandleArr: function() {
        ...
        if (!cc.g.utils.judgeArrayEmpty(handlePaiArray)) {
            ...
            newHanldeArr.forEach((code)=>{
                if ((code == WangCodeUtils.guiCode1) ||
                    (code == WangCodeUtils.guiCode2) ||
                    (code == WangCodeUtils.guiCode3)) {
                    hzArr.push(code)
                    // 删除红中
    ...
    doHiddenZhuaMa: function() {
        // this.Node_Zhua_Ma.active = false;
    },
    doNewPiaoClick:function (event, customEventData) {
        ...
        if (customEventData == 100) {
            ...
            // this.Node_Piao.active = true
            this.gameMgr.sendOp(DEF.PlayerOpt.Piao.v, customEventData);
        }
    },
    ...
    onDbgBtn: function (event, customEventData) {
        ...
        this.doRealReciveZhuaMa([2, 19, 12, 7])
