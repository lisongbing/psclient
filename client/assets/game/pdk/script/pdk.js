...
cc.Class({
    ...
    onLoad () {
        ...
        this.gameMgr = cc.g.pdkMgr;

        this._super();

        this.pGame = cc.g.pdkMgr;
    ...
    initView: function (info) {
        ...
        // 快速开始按钮
        this.Button_start = cc.find("Button_start", r);
        // 邀请好友
    ...
    initSendCard: function () {
        ...
        o.toPos = [];
        o.toPos.push(cc.find("p0", o.r).getPosition());
    ...
    initPlayerView: function () {
        ...
        let num = this.pGame.roomInfo.total;
        let seat = seatN[num] ? seatN[num] : [0,1,2,3,4,5,6];

        this.playerView = [];

        for (let i = 0; i < seat.length; ++i) {
            let view = new ctrls.PlayerView();
            view.init(pvn[seat[i]], i, this);

            this.playerView.push(view);
    ...
    upFangzhu: function () {
        ...
        if (ri.status == DEF.RMSTA.Free.v) {
            if (ri.curGameNum>0 || ri.pnd.sx==ri.pnd.xx || !eq64(this.pGame.roomInfo.owner, this.pGame.selfUID)) {
                this.Button_start.active = false;
                //this.ButtonReady.x = 0;
            } else {
                this.Button_start.active = true;
                //this.ButtonReady.x = -136;
                this.pGame.onPlayerReady('upPage');
            }
        } else {
            this.Button_start.active = false;
        }
    ...
    upOperate: function () {
        ...
        this.Node_opt.active = this.playerView[0].player.canOutCard;
        //this.Button_chupai.active = this.playerView[0].player.canOutCard;
    },
    ...
    onButtonInvite: function (event, customEventData) {
        ...
        let name = gameTypes[ri.gameType] ? gameTypes[ri.gameType].NAME : '跑得快';
    ...
    showOutChoose: function (grps, fun) {
        ...
        for (let i = 0; i < grps.length; ++i) {
            ...
            cc.g.utils.addClickEvent(bg, this.node, 'pdk', 'onClickCardsType', i);
