...
cc.Class({
    ...
    onLoad () {
        ...
        this.gameMgr = cc.g.pdknjMgr;

        this._super();

        this.pGame = cc.g.pdknjMgr;
    ...
    initOperate: function () {
        ...
        // 天关
        this.Button_tianguan = cc.find("Button_tianguan", r);
        // 提示
    ...
    upOperate: function () {
        ...
        if (ri.status == DEF.RMSTA.Tianguan.v) {
            this.Node_opt.active = p.canTg;
            this.Button_tishi.active = this.Button_chupai.active = false;
            this.Button_tianguan.active = this.Button_pass.active = true;
            return;
        }

        this.Node_opt.active = p.canOutCard;

        this.Button_tianguan.active = false;
    ...
    onButtonOperate: function (event, customEventData) {
        ...
        // 天关
        if (o == DEF.OBK.tianguan) {
            this.pGame.sendOp(DEF.PlayerOpt.Tianguan.v, [2]);
            this.Node_opt.active = false;
            return;
        }
        // 提示
    ...
    showOutChoose: function (grps, fun) {
        ...
        for (let i = 0; i < grps.length; ++i) {
            ...
            cc.g.utils.addClickEvent(bg, this.node, 'pdknj', 'onClickCardsType', i);
