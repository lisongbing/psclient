var HuPaiUtils = {
    ...
    showHuPaiPrompt:function(handleArr,putoutCards,que,rules){
        ...
        this.result = [];
		////cc.log(rules);
		////cc.log("初始的gui:"+this.guiValues)
        // //cc.log('showHuPaiPrompt start \n\n\n\n')
        // //cc.log('handleArr-->' + JSON.stringify(handleArr))
        // //cc.log('handLen-->' + handLen)
        // //cc.log('putoutCards-->' + JSON.stringify(putoutCards))
        // //cc.log('que-->' + que)
        // //cc.log('showHuPaiPrompt end \n\n\n\n')
        this.analyseRules(rules)
		this.realGuiValue = [];
		this.guiValues.forEach((value,key) =>{
			var index = 0;
			for (var i = index;i<handleArr.length;i++){
				if (handleArr[i] == value){
					let v = handleArr.splice(i,1);
					this.realGuiValue.push(v);
					i--;
					continue;
				}
			}
        ...
        let handLen = (handleArr.length + this.realGuiValue.length)%3;
		if (handLen!=1){
			return result;
		}
        ...
        hongZhongNum = this.realGuiValue.length; //红中的数量
		...
		for (var i=0;i<this.realGuiValue.length;i++){
			 handleArr.push(50);
			//  //cc.log("手牌:",handleArr);
		}
		// //cc.log("test0:",this.realGuiValue);
		// //cc.log("test1:",this.guiValues);
		// //cc.log("test:",handleArr.length);
		...
		// //cc.log("hou的gui:"+this.guiValues)
		
        //是否是没有听用
        if (this.computeWuTingYong(handleArr,putoutCards)){
            // //cc.log('result==>' + JSON.stringify(result))
            // 使用箭头函数，否则，this，指向错误
            let putOutHaveYiJi = false;
            putoutCards.forEach(v =>{
                if (v.length == 3){
                    for (let i = 0;i<v.length;i++){
                        for (let k = 0;k < this.guiValues.length;k++){
							if (v[i] == this.guiValues[k]){
							  putOutHaveYiJi = true;
							  break;
							}
						}
            ...
            newHuMap.forEach((value,key) => {
                if (key != 1 && !putOutHaveYiJi){
		
					result[key]+=this.mingtangToFun["无听用"] + (this.guiValues.length == 2?1:0)
		...
		//把红中替换掉
		for (let i=0;i<this.guiValues.length;i++){
			// //cc.log("鬼是:"+this.guiValues[i]);
			result[this.guiValues[i]]  = result[50];
		}
    ...
    computQiduiFun:function(cards,putOutCards,hucard,result){
       ...
       // fun +=this.computeJiangDui(cards, putOutCards,true)
        ...
        //门清
        if (this.mengQing){
            fun += this.computeMengQing(cards, putOutCards,false)
        }

        //中张
        if (this.zhongZhang){
            fun +=this.computeNoYaoJiui(cards, putOutCards,false);
        }
    ...
    //门清
    computeMengQing:function(cards,putOutCards,isQidui){
        let HongZhong = 50;
        if (isQidui) { //已经是七对了
            return this.mingtangToFun["门清"]
        }
        //摆牌的情况
        for (var i=0;i<putOutCards.length;i++){
           if (putOutCards[i].length== 5) { //杠的
                if (putOutCards[i][0] != 1){
                    return 0;
                }
            }else{
				return 0;
...
//中张
computeNoYaoJiui:function(cards,putOutCards,isQidui){
    let HongZhong = 50;
    if (isQidui) { //已经是七对了
        for (var i = 0; i < 7; i++ ){
            if (cards[i][0]%10 == 1 || cards[i][0]%10 == 9 ) {
                return 0 //不是将对
            }
        }
        return this.mingtangToFun["中张"]
    }
    //摆牌的情况
    for (var i=0;i<putOutCards.length;i++){
        if (putOutCards[i].length == 3) { //碰的或者飞的
            if (putOutCards[i][0]%10 == 1 || putOutCards[i][0]%10 == 9) {
                return 0
            }
        } else if (putOutCards[i].length== 5) { //杠的
            if (putOutCards[i][1]%10 == 1 || putOutCards[i][1]%10 == 9){
                return 0
            }
        }
    }
    //手牌的情况--必须是大对子才可能是将对
    for (var i=0;i<cards.length;i++) {
        if (cards[i].length == 3 &&(cards[i][0]%10 == 1 || cards[i][2]%10 == 9)){
            return 0
    ...
    computeJinGouDiao:function(cards){
        if (cards.length == 1 && this.jingoutiao) {
    ...
    computeWuTingYong:function(cards,putoutCards){
        ...
        for (var i=0;i<putoutCards.length;i++){
            for(var j=0;j<putoutCards[i].length;j++){
				for (let k = 0; k<this.guiValues.length;k++){
					if (putoutCards[i][2]==this.guiValues[k]){
						return false
					}
				}
