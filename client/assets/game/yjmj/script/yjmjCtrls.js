...
let MajhPlayerView = cc.Class({
    ...
    initView: function () {
        ...
        // 报叫
        this.Bao_Jiao_Sprite = cc.find("Bao_Jiao_Sprite", r);
        ...
        this.Sprite_offline.bg = cc.find("bg", hr);
        this.Sprite_offline.bg.active = this.Sprite_offline.active;
        this.Sprite_offline.time = cc.find("bg/time", hr).getComponent(cc.Label);
    ...
    upView: function () {
        ...
        if (!d) {
            ...
            if (this.offlineSch) {
                this.Sprite_offline.active = false;
                this.Sprite_offline.bg.active = false;
...
let MajhHandCardView = cc.Class({
    ...
    initView: function () {
        ...
        // 回放时，其他玩家的可视手牌区域
        // this.Node_hcBackPlayRoot = cc.find("Node_hcBackPlay", r);
        // this.Node_hcBackPlay = cc.find("holds", this.Node_hcBackPlayRoot);
        // this.Node_hcBackPlay.removeAllChildren(true);
        ...
        if(this.selfView.player){
            ...
            if(isZhuang && this.selfView.index == 0) {
                if (!cc.g.utils.judgeArrayEmpty(this.hcGroups)) {
                    ...
                    let nodeY = lastGrpNode.y;

                    // if (lastGrpNode && ((this.selfView.index == 0) || (this.selfView.index == 2))) {
                    //     lastGrpNode.endPosX = lastGrpNode.x + 10;
                    //     lastGrpNode.needAnim = true;
                    //     lastGrpNode.setPosition(lastGrpNode.x + 10, nodeY);
                    // }
    ...
    //             if (index == 0) {
    //                 cc.find("Sprite_cardVal", cardNode).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame('majh_cardval_' + gui);
    //             } else if (index == 1) {
    //                 cc.find("Sprite_cardVal", cardNode).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame('ri_majh_cardval_' + gui);
    //             } else if (index == 2) {
    //                 cc.find("Sprite_cardVal", cardNode).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame('to_majh_cardval_' + gui);
    //             } else if (index == 3) {
    //                 cc.find("Sprite_cardVal", cardNode).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame('le_majh_cardval_' + gui);
    //             }
    //             cardNode.code = gui
    //
    //             // 显示提的牌
    ...
    //                 if (!cc.g.utils.judgeObjectEmpty(Sprite_topMeng)) {
    //                     Sprite_topMeng.active = true;
    //                     Sprite_topMeng.yellowZe = true;
    //                 }
    //             }
    //
    //             break;
    ...
    changeHandleCardZhong: function(code, gui, index) {
        ...
        for(let i=0; i<grp.length; i++){
            ...
            if (cardNode.active && (code == cardNode.code)) {
                ...
                if (this.selfView.index == 0) {
                    let Sprite_Que = cc.find("Sprite_Que", cardNode)
                    Sprite_Que.active = false
                    cardNode.isQue = false
                    let Sprite_topMeng = cc.find("Sprite_topMeng", cardNode)
                    if (!cc.g.utils.judgeObjectEmpty(Sprite_topMeng)) {
                        Sprite_topMeng.active = false;
                        Sprite_topMeng.yellowZe = false;
                    }
                }
    ...
    changeHandleCardZhongTwo: function(gui, index) {
        let grp = this.hcGroups;
        for(let i=0; i<grp.length; i++){
            let cardNode = grp[i];
            if (!cardNode.active) {
                ...
                if(cardNode.active){
                    cardNode.runAction(
                        cc.sequence(
                            ...
                            cc.callFunc(
                                function (params) {
                                    if (i == grplen && this.gameMgr.roomInfo.NewRlue.indexOf(21) == -1) {
    ...
    showHuPaiView: function (palyerViewItem, code) { // 显示胡牌
        ...
        // 显示梦伴
        let Sprite_topMeng = cc.find("Sprite_Hu/Sprite_topMeng", card);
        Sprite_topMeng.active = false;
    ...
    onTouchstart: function (event) {
        ...
        let tong = this.pPage.getYaoTongGui()
        if (tong) {
            // if (parseInt(tg.code) == 1 || parseInt(tg.code) == 11 || tg.zezao || this.pPage.isAutoHu) {
            //     return
            // }
            if (parseInt(tg.code) == 1 || parseInt(tg.code) == 11 || this.pPage.isAutoHu) {
                return
            }
        } else {
            // if (parseInt(tg.code) == 1 || tg.zezao || this.pPage.isAutoHu) {
            //     return
            // }

            if (parseInt(tg.code) == 1 || this.pPage.isAutoHu) {
                return
            }
        }
...
let MajhPongCardView = cc.Class({
    ...
    // 提操作
    doChangeZhongPai: function(code, gui) {
        ...
        for (let i = 0; i < this.pongPaiArr.length; i++) {
            ...
            if (lastOneItem.code == code) {
                if (lastOneItem.type == 'fei' || lastOneItem.type == 'peng') {
                    lastOneItem.type = 'peng'
                    let Layout_Top = cc.find("Layout_Top", lastOneItem)
                    let code0 = lastOneItem.code;
                    let code1 = lastOneItem.code1;
                    let code2 = lastOneItem.code2;
                    ...
                    if (tong) {
                        if (code0 == gui) {
                            ...
                            lastOneItem.code = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Peng0/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        } else if (code1 == gui) {
                            ...
                            lastOneItem.code1 = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Peng1/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        } else if (code2 == gui) {
                            ...
                            lastOneItem.code2 = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Peng2/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        }
                    } else {
                    ...
                    break;
                } else if (lastOneItem.type == 'mkang') {
                    let Layout_Top = cc.find("Layout_Top", lastOneItem)
                    let code0 = lastOneItem.code;
                    let code1 = lastOneItem.code1;
                    let code2 = lastOneItem.code2;
                    let code3 = lastOneItem.code3;
                    if (tong) {
                        if (code0 == gui) {
                            ...
                            lastOneItem.code = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Gang0/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        } else if (code1 == gui) {
                            ...
                            lastOneItem.code1 = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Gang1/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        } else if (code2 == gui) {
                            ...
                            lastOneItem.code2 = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Gang2/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        } else if (code3 == gui) {
                            ...
                            lastOneItem.code3 = code
                            // 显示提的牌
                            let Sprite_topMeng = cc.find("Sprite_Gang3/Sprite_topMeng", Layout_Top)
                            Sprite_topMeng.active = false;
                            Sprite_topMeng.yellowZe = false;
                        }
                    } else {
                    ...
                    break;
                } else if (lastOneItem.type == 'akang') {
                    let Layout_Top = cc.find("Layout_Top", lastOneItem)
                    let code0 = lastOneItem.code;
                    let code1 = lastOneItem.code1;
                    let code2 = lastOneItem.code2;
                    ...
                    if (tong) {
                        ...
                        if (lastOneItem.code == 1) {
                            hNum++;
                        }
                        if (lastOneItem.code == 11) {
                            tNum++;
                        }
                        if (lastOneItem.code1 == 1) {
                            hNum++;
                        }
                        if (lastOneItem.code1 == 11) {
                            tNum++;
                        }
                        if (lastOneItem.code2 == 1) {
                            hNum++;
                        }
                        if (lastOneItem.code2 == 11) {
                            tNum++;
                        }
                        if (lastOneItem.code3 == 1) {
                            hNum++;
                        }
                        if (lastOneItem.code3 == 11) {
                            tNum++;
                        }
                    } else {
                        if (code0 == 1) {
                    ...
                    if (this.selfView.index == 0) {
                        ...
                        if ((hNum > 0) || (tNum > 0)) {
                            cc.find("Sprite_Gang0/Sprite_Hong_bg", Layout_Top).active = true;
                            cc.find("Sprite_Gang0/Label_Zhong", Layout_Top).active = true;
                        } else {
                            cc.find("Sprite_Gang0/Sprite_Hong_bg", Layout_Top).active = false;
                            cc.find("Sprite_Gang0/Label_Zhong", Layout_Top).active = false;
                        }
                        if ((hNum > 0) && (tNum > 0)) {
                            cc.find("Sprite_Gang0/Label_Zhong", Layout_Top).getComponent(cc.Label).string = hNum + "鸡" + tNum+ "筒";
                        } else if (hNum > 0) {
                            cc.find("Sprite_Gang0/Label_Zhong", Layout_Top).getComponent(cc.Label).string = hNum + "鸡"
                        } else if (tNum > 0) {
                            cc.find("Sprite_Gang0/Label_Zhong", Layout_Top).getComponent(cc.Label).string = tNum + "筒"
                        }

                    } else if (this.selfView.index == 1) {

                        if ((hNum > 0) || (tNum > 0)) {
                            cc.find("Sprite_Gang3/Sprite_Hong_bg", Layout_Top).active = true;
                            cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).active = true;
                        } else {
                            cc.find("Sprite_Gang3/Sprite_Hong_bg", Layout_Top).active = false;
                            cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).active = false;
                        }

                        // cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).getComponent(cc.Label).string = hNum + "鸡";

                        if ((hNum > 0) && (tNum > 0)) {
                            cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).getComponent(cc.Label).string = hNum + "鸡" + tNum+ "筒";
                        } else if (hNum > 0) {
                            cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).getComponent(cc.Label).string = hNum + "鸡"
                        } else if (tNum > 0) {
                            cc.find("Sprite_Gang3/Label_Zhong", Layout_Top).getComponent(cc.Label).string = tNum + "筒"
                        }
                    } else if (this.selfView.index == 2) {
    ...
    // },
    // 手里的牌，直接就杠了
    doAddGangPai: function(codeArr, index) {
        // let positionX, positionY
        ...
        cc.find("Sprite_Gang0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + codeArr[1]);
        cc.find("Sprite_Gang1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + codeArr[2]);
        cc.find("Sprite_Gang2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + codeArr[3]);
        cc.find("Sprite_Gang3/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + codeArr[4]);



        newCard.code = codeArr[1]
        newCard.code1 = codeArr[2]
        newCard.code2 = codeArr[3]
        newCard.code3 = codeArr[4]
        newCard.type = 'mkang'



        let Sprite_ZeZao0 = cc.find("Sprite_Gang0/Sprite_topMeng", Layout_Top)
        let Sprite_ZeZao1 = cc.find("Sprite_Gang1/Sprite_topMeng", Layout_Top)
        let Sprite_ZeZao2 = cc.find("Sprite_Gang2/Sprite_topMeng", Layout_Top)
        let Sprite_ZeZao3 = cc.find("Sprite_Gang3/Sprite_topMeng", Layout_Top)

        let tong = this.pPage.getYaoTongGui()
        if (tong) {
            if ((newCard.code == 1) || (newCard.code == 11)) {
                Sprite_ZeZao0.active = true
            } else {
                Sprite_ZeZao0.active = false
            }
            if ((newCard.code1 == 1) || (newCard.code1 == 11)) {
                Sprite_ZeZao1.active = true
            } else {
                Sprite_ZeZao1.active = false
            }
            if ((newCard.code2 == 1) || (newCard.code2 == 11)) {
                Sprite_ZeZao2.active = true
            } else {
                Sprite_ZeZao2.active = false
            }
            if ((newCard.code3 == 1) || (newCard.code3 == 11)) {
                Sprite_ZeZao3.active = true
            } else {
                Sprite_ZeZao3.active = false
            }
        } else {
            if (newCard.code == 1) {
                Sprite_ZeZao0.active = true
    ...
    //         cc.find("Sprite_Peng0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
    //         cc.find("Sprite_Peng1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
    //         cc.find("Sprite_Peng2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + 1);
    //     }
    ...
    doAddOnePongPai: function(cardObj) {
        ...
        if (cardObj.type == 'peng') {
            cc.find("Sprite_Peng0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
            cc.find("Sprite_Peng1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code1);
            cc.find("Sprite_Peng2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code2);

            let Sprite_ZeZao0 = cc.find("Sprite_Peng0/Sprite_topMeng", Layout_Top)
            let Sprite_ZeZao1 = cc.find("Sprite_Peng1/Sprite_topMeng", Layout_Top)
            let Sprite_ZeZao2 = cc.find("Sprite_Peng2/Sprite_topMeng", Layout_Top)

            if (tong) {
                if ((cardObj.code == 1) || (cardObj.code == 11)) {
                    Sprite_ZeZao0.active = true
                } else {
                    Sprite_ZeZao0.active = false
                }
                if ((cardObj.code1 == 1) || (cardObj.code1 == 11)) {
                    Sprite_ZeZao1.active = true
                } else {
                    Sprite_ZeZao1.active = false
                }
                if ((cardObj.code2 == 1) || (cardObj.code2 == 11)) {
                    Sprite_ZeZao2.active = true
                } else {
                    Sprite_ZeZao2.active = false
                }
            ...
            cc.find("Sprite_Gang0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
            cc.find("Sprite_Gang1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code1);
            cc.find("Sprite_Gang2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code2);
            cc.find("Sprite_Gang3/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code3);
            ...
            if (tong) {
                ...
                if ((cardObj.code3 == 1) || (cardObj.code3 == 11)) {
                    Sprite_ZeZao3.active = true
                } else {
                    Sprite_ZeZao3.active = false
                }
        ...
        newCard.code = cardObj.code
        newCard.code1 = cardObj.code1
        newCard.code2 = cardObj.code2
        newCard.code3 = cardObj.code3
        newCard.type = cardObj.type
        // if (cardObj.type == 'fei') {
        //     newCard.guiCode = cardObj.guiCode
        // }
        newCard.gtype = cardObj.gtype
        // 放入数组
        this.pongPaiArr.push(newCard);
            ...
            for(let j=0; j<cards.length; j++){
                ...
                if (nodePong.type == cardItem.type) {
                    ...
                    if (cardItem.type == 'peng') {
                        ...
                        cc.find("Sprite_Peng0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + cardItem.code);
                        cc.find("Sprite_Peng1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + cardItem.code);
                        cc.find("Sprite_Peng2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + 1);
                    }
...
let D2SettleView = cc.Class({
    ...
    initView: function () {
        ...
        // 宁远麻将
        this.Node_Ny_View = cc.find("Node_Ny_View", r);
        this.Node_Ny_View.active = true;


        this.Node_Ma = cc.find("Node_Ma", this.Node_Ny_View);
        // ma view
        this.Layout_Hor = cc.find("Node_Ny_View/Node_Ma/Layout_Hor", r);

        // 分享一下
        cc.find("Button_share", r).active = false;
        this.Button_share = cc.find("Button_share", this.Node_Ny_View);
        this.Button_share.on('touchend', this.share, this);

        // 查看剩下牌
        cc.find("Button_sp", r).active = false;
        this.Button_sp = cc.find("Button_sp", this.Node_Ny_View);
        this.Button_sp.on('touchend', this.onsp, this);


        // 继续游戏
        cc.find("Button_gogame", r).active = false;
        this.Button_gogame = cc.find("Button_gogame", this.Node_Ny_View);
    ...
    upPlayers: function () {
        ...
        let str = '幺鸡麻将'//cc.g.areaInfo[ri.origin].name + '麻将';
        // let str = '红中麻将';
        str += ' 房间号：' + ri.roomId;
        ...
        let maValue = settleData.maValue
        for (let i = 0; i < 10; i++) {
            ...
            if (allPerResult[i]) {
                ...
                this.crtPlayerItem(vbPlayerContent, allPerResult[i], maValue);
            }
        }
    ...
    // 玩家
    crtPlayerItem: function (vbPlayerContent, allPerResultItem, maValue) {
        // // 头像 名字 ID
        ...
        // 显示票
        let piao = false
        let uid = allPerResultItem.uid
        if (eq64(maValue[0], uid)) {
            piao = true
        } else {
            piao = false
        }
        ...
        if (piaoView.active) {
            piaoView.getComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('yjmj_ma');
        }
        ...
        if (!cc.g.utils.judgeArrayEmpty(mingtang)) {
            ...
            for (let i = 0; i < mingtang.length; i++) {
                ...
                if (ming == 7) {
                    if (guiNum == 1) {
                        spr.addComponent(cc.Sprite).spriteFrame =  this.pg.majhAtlas0.getSpriteFrame('jiesuan_gui_1');
                    } else if (guiNum == 2) {
                        spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('jiesuan_gui_2');
                    } else if (guiNum == 3) {
                        spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('jiesuan_gui_3');
                    } else if (guiNum == 4) {
                        spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('jiesuan_gui_4');
                    }
                    guiNode = spr
                } else {
                    spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('mingtang_yj_' + mingtang[i]);
        ...
        if (cc.g.utils.judgeObjectEmpty(win)) {
            win = 0;
        }
    ...
    showMaFunc: function() {
        ...
        if (!cc.g.utils.judgeArrayEmpty(maValue)) {
            ...
            for (let i = 1; i < maValue.length; ++i) {
                ...
                let sprite_cardVal_img = cc.find("Sprite_cardVal", cardNode);
                sprite_cardVal_img.getComponent(cc.Sprite).spriteFrame = this.pg.majhCardAtlas0.getSpriteFrame('majh_cardval_' + code);
                this.Layout_Hor.addChild(cardNode);
            }
            this.Layout_Hor.getComponent(cc.Layout).updateLayout();
            // this.svLeftcards.getComponent(cc.ScrollView).scrollToTop();
        }
    },
...
let D2SettleFinalView = cc.Class({
    ...
    upView: function () {
        ...
        // 地区
        this.Label_area.string = '幺鸡麻将'//cc.g.areaInfo[ri.origin].name + '麻将';
        // 房间号
