...
var WangCodeUtils = require("nymjWangCodeUtils");
var HuPaiUtils = require("nymjHuPaiUtils");
var nymjCtrls = require('nymjCtrls');
var DEF = require('nymjDef');
...
cc.Class({
    ...
    initPaiCount: function() {
        ...
        let zhuanNum = 1;
        let hongZhongWangCount = this.getHongZhongWangCount();
        // 如果是红中当王，则庄家手牌不减1，其他减1
        if (hongZhongWangCount > 0) {
            zhuanNum = 0;
        }

        //  抓马数量
        let zhuaMaCount = this.getZhuMaCount()

        let showCount = 108 - 14 - (palerCount - 1) * 13 - zhuanNum + hongZhongWangCount - zhuaMaCount;
    ...
    getHongZhongWangCount: function() {
        // 找红中个数  12 无红中 13 4个 14 8个 15 11个 16 12个
        let newRlue = this.gameMgr.roomInfo.NewRlue
        let hongZhongCount = 0;
    ...
    initPlayerView: function () {
        ...
        while (true) {
            ...
            let view = new nymjCtrls.PlayerView();
    ...
    onButtonCopyRoom: function (event, customEventData) {
       ...
       if (ri.clubId) {
        ...
        // des += cc.g.areaInfo[ri.origin].name + '宁远麻将,';
        des += '宁远麻将,';
        des += ri.GameNum + '局,';
        des += ri.total + '人';
    ...
    onButtonInvite: function (event, customEventData) {
        ...
        let name = gameTypes[ri.gameType] ? gameTypes[ri.gameType].NAME : '宁远麻将';
    ...
    showSelectTiView: function(arr) {
        ...
        arr.forEach((code)=>{
            ...
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'nymj', 'onClickTiClicked', cardNode);
            // nodejs
    ...
    showSelectGangView: function(arr) {
        ...
        arr.forEach((code)=>{
            ...
            lastOneX = viewIndex *  DEF.hcMultHuTiPos[0].moveBy.z + (viewIndex + 1) * (DEF.hcMultHuTiPos[0].moveBy.z/2)
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'nymj', 'onClickGangClicked', cardNode);
            // nodejs
            this.huTi_Node_Layout.addChild(cardNode, 1, 'Node_card_Num'+viewIndex);
            // get card node
            itemArr.push(cardNode)
            ...
            if (canGangCount > 1) {
                ...
                if (!cc.g.utils.judgeArrayEmpty(repeatArr)) {
                    repeatArr.forEach((anItem)=>{
                        if (!cc.g.utils.judgeArrayEmpty(saveGangArr)) {
                            let findCode = false
                            for (let i = 0; i < saveGangArr.length; i++) {
                                let scode = saveGangArr[i]
                                if (scode == parseInt(anItem.code)) {
                                    findCode = true
                                    break
                                }
                            }
                ...
                if (!cc.g.utils.judgeArrayEmpty(allGangArr)) {
                    if (allGangArr.length == 1) {
                        this.gameMgr.sendOp(DEF.PlayerOpt.Gang.v, allGangArr[0]);
                        this.doHiddenHutiGang();
                    } else {
                        // 杠牌
    ...
    doClickPeng: function () {
        ...
        if (this.Button_Gang.active) {
            ...
            if (!cc.g.utils.judgeArrayEmpty(gangArr)) {
                // for (let i= 0; i < gangArr.length; i++) {
                //     let code = gangArr[i]
                //     if (canOptVal == code) {
                //         saveGangArr.push(code)
                //         break
    ...
    onGameSettlement: function () {
        if (! this.settleView) {
            this.settleView = new nymjCtrls.SettleView();
    ...
    onGameSettleFinal: function () {
        if (! this.settleFinalView) {
            this.settleFinalView = new nymjCtrls.SettleFinalView();
        ...
        if (!cc.g.utils.judgeArrayEmpty(this.tingCodeArr)) {
          ...
          //  let tingIndex = WangCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);
            ...
            for (let i = 0; i < this.tingCodeArr.length; i++) {
                ...
                handlePaiArrayActive.forEach(org => {
                    let Sprite_topMeng = cc.find("Sprite_Wang", org)
                    if ((parseInt(code) == parseInt(org.code)) || (parseInt(org.code) == 50)) {
                        Sprite_topMeng.active = true;
                        org.yellowZe = true
                    }
                });
    ...
    doCheckHuPai: function(reconnect=false) {
        ...
        if (!cc.g.utils.judgeArrayEmpty(resultArr)) {
            ...
            // 红中总个数
            let allHongZhong = this.getHongZhongWangCount() - hzCount;
            ...
            resultArrBig.forEach((huAler)=>{
                handlePaiArray.forEach((node)=>{
                    if (node.active && (huAler.outCode == node.code) && (node.code != 50) && (node.code != WangCodeUtils.guiCode1)
                        && (node.code != WangCodeUtils.guiCode2) && (node.code != WangCodeUtils.guiCode3)) {
        ...
        if (this.huPaiCurrentItem) {
            ...
            if (huMapArr.length <= 18) {
                ...
                huMapArr.forEach((item) => {
                    ...
                    //
                    let Sprite_topMeng = cc.find("Sprite_Wang", cardNode);
                    if ((code == WangCodeUtils.guiCode1) || (code == WangCodeUtils.guiCode2) || (code == WangCodeUtils.guiCode3)) {
                        Sprite_topMeng.active = true;
                    } else {
                        Sprite_topMeng.active = false;
                    }
