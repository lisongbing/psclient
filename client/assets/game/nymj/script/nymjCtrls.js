let DEF = require('nymjDef');
var TingCodeUtils = require("nymjWangCodeUtils");
...
let MajhHandCardView = cc.Class({
    ...
    onTouchstart: function (event) {
        ...
        if (this.pPage.isAutoHu) {
            return;
        }

        let canOutGui = TingCodeUtils.getTingYongCount(this.gameMgr.roomInfo.NewRlue);
        if (!canOutGui) {
...
let MajhPongCardView = cc.Class({
    ...
    doAddOnePongPai: function(cardObj) {
        ...
        if (cardObj.type == 'peng') {
            ...
            cc.find("Sprite_Peng0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
            cc.find("Sprite_Peng1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.code);
            cc.find("Sprite_Peng2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = this.pPage.majhCardAtlas0.getSpriteFrame(cardKeyName + cardObj.guiCode);
            // let Sprite_topMeng = cc.find("Sprite_Peng2/Sprite_topMeng", Layout_Top)
            // Sprite_topMeng.active = true;
        }
            ...
            for(let j=0; j<cards.length; j++){
                ...
                if (nodePong.type == cardItem.type) {
                    ...
                    if (cardItem.type == 'peng') {
                        ...
                        cc.find("Sprite_Peng0/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + cardItem.code);
                        cc.find("Sprite_Peng1/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + cardItem.code);
                        cc.find("Sprite_Peng2/Sprite_Val", Layout_Top).getComponent(cc.Sprite).spriteFrame = atlas.getSpriteFrame(cardKeyName + nodePong.guiCode);
                    }
...
let D2SettleView = cc.Class({
    ...
    crtPlayerItem: function (vbPlayerContent, allPerResultItem) {
        ...
        let piao = cc.find("Sprite_Piao", vbPlayerContent);
        let Sprite_Header_Play_Vaule = cc.find("Sprite_Header_Play_Vaule", vbPlayerContent);
        let Sprite_Header_Piao_Bg = cc.find("Sprite_Header_Piao_Bg", vbPlayerContent);
        ...
        if (!cc.g.utils.judgeObjectEmpty(playerViewItemTwo)) {
            ...
            if (piao > 0) {
                Sprite_Header_Piao_Bg.active = true;
                Sprite_Header_Play_Vaule.active = true;
                Sprite_Header_Piao_Bg.getComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('ny_piao_small_bg');
                Sprite_Header_Play_Vaule.getComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('ny_piao_small_'+piao);
            } else if (piao == 0) {
                Sprite_Header_Piao_Bg.active = true;
                Sprite_Header_Play_Vaule.active = false;
                Sprite_Header_Piao_Bg.getComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('ny_bupiao_small');
                //
                // Sprite_Header_Play_Vaule.getComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('ny_piao_small_'+piao);
            } else {
                Sprite_Header_Piao_Bg.active = false;
                Sprite_Header_Play_Vaule.active = false;
            }
        }
        ...
        if (!cc.g.utils.judgeArrayEmpty(mingtang)) {
            ...
            for (let i = 0; i < mingtang.length; i++) {
                ...
                if (guiNum > 0) {
                    ...
                    spr.addComponent(cc.Sprite).spriteFrame = this.pg.majhAtlas0.getSpriteFrame('mingtang_ny_' + mingtang[i]);
        ...
        if (!cc.g.utils.judgeArrayEmpty(hand)) {
            ...
            for (let i = 0; i < hand.length; i++) {
                ...
                if (eq64(this.pg.gameMgr.selfUID, allPerResultItem.uid)) {
                    // cc.dlog('cc.g.userMgr.userId ---- ', cc.g.userMgr.userId);
                    // cc.dlog('allPerResultItem.uid ---- ', allPerResultItem.uid);
                    this.isselfwin = (win > 0);
                    // cc.dlog('this.isselfwin ---- ', this.isselfwin);
                }
                ...
                if ((cardcode == TingCodeUtils.guiCode1) || (cardcode == TingCodeUtils.guiCode2) || (cardcode == TingCodeUtils.guiCode3)) {
                    Sprite_topMeng.active = true;
                } else {
                    Sprite_topMeng.active = false;
                }
        ...
        Button_Detail.on('touchend', ()=>{
            self.showDetail(details, curname, false)
