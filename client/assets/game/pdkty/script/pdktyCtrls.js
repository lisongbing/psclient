...
let LocSettleView = cc.Class({
    ...
    crtPlayerItem: function (idx) {
        ...
        // 鸟 飘
        let Sprite_niao = cc.find("Sprite_niao", r).getComponent(cc.Sprite);
        let Sprite_piao = cc.find("Sprite_piao", r).getComponent(cc.Sprite);
        Sprite_piao.val = cc.find("Sprite_piao/val", r).getComponent(cc.Sprite);

        Sprite_niao.node.active = pla.daniao > 0;
        Sprite_piao.node.active = pla.piaofen > 0;
        if (pla.piaofen) {
            Sprite_piao.val.spriteFrame = PG.pageAtlas0.getSpriteFrame('pdkty_pt_'+pla.piaofen);
        }
    ...
    show: function () {
        ...
        // 地区
        this.Label_diqu.string = '通用跑得快'; //cc.g.areaInfo[ri.origin].name + '跑得快';
        // 房间号
...
let LocSettleFinalView = cc.Class({
    ...
    upPlyaers: function () {
        ...
        for (let i = 0; i < sd.player.length; ++i) {
            ...
            Sprite_niao.node.active = d.daniao > 0;
            //Sprite_piao.node.active = pla.piaofen > 0;
            //if (pla.piaofen) {
            //    Sprite_piao.val.spriteFrame = PG.pageAtlas0.getSpriteFrame('pdkty_pt_'+pla.piaofen);
            //}
...
let LocPlayerView = cc.Class({
    ...
    initView: function () {
        ...
        // 鸟
        this.Sprite_niao = cc.find("Sprite_niao", r).getComponent(cc.Sprite);
        // 飘
        this.Sprite_piao = cc.find("Sprite_piao", r).getComponent(cc.Sprite);
        this.Sprite_piao.val = cc.find("Sprite_piao/val", r).getComponent(cc.Sprite);
    ...
    upView: function () {
        ...
        if (!d) {
            ...
            this.Sprite_niao.node.active = false;
            this.Sprite_piao.node.active = false;
    ...
    upNiaoPiao: function () {
        ...
        let p = this.player;
        if (GM.pf_dn == 'dn') {
            this.Sprite_niao.node.active = p.daniao > 0;
        } else if (GM.pf_dn == 'pf') {
            this.Sprite_piao.node.active = p.piaofen > 0;
            if (p.piaofen) {
                this.Sprite_piao.val.spriteFrame = PG.pageAtlas0.getSpriteFrame('pdkty_pt_'+p.piaofen);
            }
        }
    },
