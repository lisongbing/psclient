...
cc.Class({
    ...
    initOptFun: function () {
        ...
        //打鸟
        kf[LG_opt.CanDaNiao.v]    = this.onCanDaNiao.bind(this);
        kf[LG_opt.DaNiao.v]    = this.onDaNiao.bind(this);

        //飘分
        kf[LG_opt.CanPiaofen.v]    = this.onCanPiaofen.bind(this);
        kf[LG_opt.Piaofen.v]    = this.onPiaofen.bind(this);
    ...
    dataPrepare: function () {
        ...
        //当玩家选中了打鸟规则,expand[0] 0 未选择打鸟,1 已选择打鸟 ;expand[1] 打鸟分数
        //当玩家选中了飘分规则,expand[0] 0 未选择飘分,1 已飘分 ;expand[1] 飘分分数
        let expand = this.gm.isBackPlayMode() ? [] : this.d.expand;

        this.daniao = this.piaofen = -1;
        if (ri.status >= DEF.RMSTA.DaNiao.v) {
            if (this.gm.pf_dn == 'dn') {
                if (expand[0]==1) {
                    this.daniao = expand[1];
                }
            } else if (this.gm.pf_dn == 'pf') {
                if (expand[0]==1) {
                    this.piaofen = expand[1];
                }
            }
        } 
    ...
    // 可以打鸟
    onCanDaNiao: function (v) {
        cc.log(this.dbgstr('onCanDaNiao 可以打鸟'));

        this.gm.roomInfo.status = DEF.RMSTA.DaNiao.v;
    ...
    // 打鸟
    onDaNiao: function (va) {
        cc.log(this.dbgstr('onDaNiao 打鸟'));

        this.gm.roomInfo.status = DEF.RMSTA.Play.v;

        let v = [];
        va.forEach(e => v.push(e.toNumber ? e.toNumber() : e));

        this.daniao = (v[0]==1 ? this.gm.dnsco : 0);

        this.view.upNiaoPiao();
    ...
    // 可以飘分
    onCanPiaofen: function (v) {
        cc.log(this.dbgstr('onCanPiaofen 可以飘分'));

        this.gm.roomInfo.status = DEF.RMSTA.PiaoFen.v;
    ...
    // 飘分
    onPiaofen: function (va) {
        cc.log(this.dbgstr('onPiaofen 飘分'));
    ...
    sendCard: function (va) {
        this.gm.roomInfo.status = DEF.RMSTA.SendCard.v;
    ...
    // 天关
    canTianguan: function (va) {
        cc.log(this.dbgstr('canTianguan 可以天关'));
        ...
        this.gm.roomInfo.status = DEF.RMSTA.Tianguan.v;
    ...
    tianGuan: function (va) {
        cc.log(this.dbgstr('tianGuan 天关'));
        ...
        if (v[0]==2) {
            this.gm.audio.tianguan(this.d.sex);
        }
    ...
    outCard: function () {
        ...
        if (this.handCodes.length > info.codes.length) {
            let newCombs = [];
            Comb.forEach(e => {
                if (e.type!=DEF.ComType.SAN && e.type!=DEF.ComType.SAN1 && e.type!=DEF.ComType.FJ) {
                    newCombs.push(e);
                }
            });
            Comb = newCombs;
    ...
    onOutCard: function (va, isClt) {
        ...
        if (v[0] < 0) {
            ...
            v[0]==-8 && cc.g.global.hint('不允许三A炸');
            v[0]==-9 && cc.g.global.hint('不允许三K炸');
            v[0]==-10 && cc.g.global.hint('不允许四不带');
            v[0]==-11 && cc.g.global.hint('不允许四带一');
            v[0]==-12 && cc.g.global.hint('不允许四带二');
            v[0]==-13 && cc.g.global.hint('不允许四带三');
            v[0]==-14 && cc.g.global.hint('不是最后一手不允许三带一');
            v[0]==-15 && cc.g.global.hint('不是最后一手不允许三同');
            v[0]==-16 && cc.g.global.hint('不是最后一手不允许飞机少带');
            v[0]==-17 && cc.g.global.hint('炸弹不可以拆');
            v[0]==-19 && cc.g.global.hint('不允许炸弹');
            v[0]==-19 && cc.g.global.hint('不允许三带一');
            v[0]==-20 && cc.g.global.hint('不允许三同');
            v[0]==-21 && cc.g.global.hint('不允许飞机少带');

            //(v[0]==-9 || v[0]==-10 || v[0]<-13 ) && cc.g.global.hint('未知错误');
