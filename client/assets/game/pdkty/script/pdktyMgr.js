...
let Logic = require('pdktyLogic');
let Audio = require('pdktyAudio');
let Player = require('pdktyPlayer');
let MsgQueue = require('pdktyMsgQueue');
let BackPlay = require('pdktyBackPlay');
...
cc.Class({
    ...
    upExpand: function () {
        ...
        for (let i = 0; i < this.roomInfo.NewRlue.length; ++i) {
            ...
            if (this.pf_dn == -1) {
                if (r>=11 && r<=13) {
                    this.pf_dn = 'dn';
                    this.dnsco = {11:10, 12:20, 13:50,}[r];
                }
                if (r>=15 && r<=19) {
                    this.pf_dn = 'pf';
                    this.arrpiao = {15:[1], 16:[2], 17:[1,2,3], 18:[2,3,5], 19:[2,5,8],}[r];
                }
            }
        }
        this.isMustOut = true;
    ...
    onWaitReadyEnd: function () {
        ...
        if (this.pf_dn == 'dn') {
            this.roomInfo.status = DEF.RMSTA.DaNiao.v;
        } else if (this.pf_dn == 'pf') {
            this.roomInfo.status = DEF.RMSTA.PiaoFen.v;
        } else {
    ...
    prepareSettleFinalData: function (resp) {
        ...
        */}
        ...
        resp.list.forEach(e => {
            ...
            sd.daniao = this.uidPlayers[sd.uid].daniao;
            sd.piaofen = this.uidPlayers[sd.uid].piaofen;
