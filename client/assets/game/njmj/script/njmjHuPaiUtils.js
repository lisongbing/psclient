...
var HuPaiUtils = {
	...
	noYiJiuAndMengQing:  false,
	yiJiuAndJiangdui :   false,
	setNoYiJiu: function(rules){
		var r;
		for (var i = 0;i<rules.length;i++){
			if (rules[i] == 41){
				this.noYiJiuAndMengQing = true;
			}else if (rules[i] == 38){
				this.yiJiuAndJiangdui = true;
			}
		}
	},
    ...
    showHuPaiPrompt:function(handleArr,putoutCards,que,rules){
       ...
       /* let jsonMapStr = JSON.stringify(result)
        ...
        // //cc.log("最终的番数",result,JSON.stringify(result));
    ...
    computQiduiFun:function(cards,putOutCards,hucard,result){
        ...
        //将对，带幺九
		if (this.yiJiuAndJiangdui){
			fun +=this.computeJiangDui(cards, putOutCards,true);
			fun +=this.computeYaoJiui(cards, putOutCards,true);
		}
		//中张/门清
		if (this.noYiJiuAndMengQing){
			fun +=this.computeNoYaoJiui(cards, putOutCards,true);
			fun +=this.computeMengQing(cards, putOutCards,true);
		}
        //清一色
    ...
    computeNotQiDuiFun:function(cards,putOutCards,hucard,result){
		...
		if (lastCard.length == 3){
			if (lastCard[0] == 1 && lastCard[1] == 2 && lastCard[2]==3&& hucard == 2){
				fun +=this.mingtangToFun["卡二条"];
			}
		}
         //将对，带幺九
/*       if (this.yiJiuAndJiangdui){
		   //将对
			...
			fun +=this.computeYaoJiui(cards, putOutCards,false);
		}
		//中张/门清
		if (this.noYiJiuAndMengQing){
	...
	//带幺九
    computeYaoJiui:function(cards,putOutCards,isQidui){
        let HongZhong = 50;
        if (isQidui) { //已经是七对了
            for (var i = 0; i < 7; i++ ){
