...
cc.Class({
    ...
    // 初始化胡数信息
    initHuInfo: function () {
        let ruleWinList = cc.g.utils.getJson('ErQiShiWinList');
        this.huInfo = {};
    ...
    getKuaiNum: function (hand, show) {
        ...
        //开
        show.forEach(e => {
            if ((e.length == 5) && (e[0]==1 && e[1]==e[2] && e[1]==e[3] && e[1]==e[4]) ){
                ++n;
            }
        });
    ...
    /* =================================================================================================================== */
    // 吃牌
    getChiBai: function(cards, code, isbai) {
    ...
    analyseChibai: function (NIfo) {
        ...
        return (chibai.length > 0) ? chibai : null;
    },
    ...
    /* =================================================================================================================== */
    // 过滤可胡牌里的开牌
    guoLvHuCodes:function (hcGrps, hures) {
        if (!hcGrps) {
            cc.log('过滤时没有手牌');
            return hures;
        }
        ...
        let kai = {};
        for (const k in hcGrps) {
            const g = hcGrps[k];

            let ifo = this.analyseGrop(g);
        ...
        if (Object.keys(kai).length < 1) {
            cc.log('没有可以开的牌跳过后续过滤');
            return hures;
        }

        // 能胡的所有牌的信息key{fan hx}
        let newhu = {};
        for (const key in hures) {
            if (!kai[key]) {
                newhu[key] = hures[key];
            }
        }
    ...
    // 获取打出后可以下叫的牌
    getJiaoCodes:function (codes, NewRlue, huxi, showGroups, hcGroups) {
        ...
        for (let i = 0; i < codes.length; ++i) {
            ...
            cc.log('打出', jc);
            let chckv = this.getCanHuCards(checkCodes, NewRlue, huxi, showGroups, hcGroups);
            let hu = [];
    ...
    // 获取可以胡的牌
    getCanHuCards: function (codes, NewRlue, huxi, showGroups, hcGroups) {
        let JHX = 10;
        let inKanFunc = function(kans,card){
			...
			else //两个的
			{
				...
				if (tail[0] ==tail[1]){//相等
					...
					if (tail[0] <= 10)
					{
                        ...
                        totalHuxi = handExceptTailHuxi+otherhuxi
                        if ((totalHuxi >= JHX) && (!inKanFunc(kan,tail[0]+10))){ {
				...
				else//不相等
				{
					...
					else if ((t[1]-t[0])==1) 
					{
						...
						{ //只能胡３,１３
							...
							if ((totalHuxi >= JHX) && !inKanFunc(kan,t[1]+1)){
							...
							if ((totalHuxi >= JHX) && !inKanFunc(kan,t[0]-1)) {
						...
						else  //胡两边的
						{
							...
							if ((totalHuxi >= JHX) && !inKanFunc(kan,t[0])) {
								this.computeFun(hand,showGroups,res,totalHuxi,t[0])
							}
					...
					else if ((t[0] == 2 && t[1] == 7) || (t[0] == 12 && t[1] == 17))
					{
						...
						if ((totalHuxi >= JHX)&& !inKanFunc(kan,t[0] + 8)) {
					...
					else if ((t[0] == 2 && t[1] == 10) || (t[0] == 12 && t[1] == 20))
					{
						...
						if ((totalHuxi >= JHX)&& !inKanFunc(kan,t[0] + 5)) {
