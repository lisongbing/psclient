...
var remoteResUrl = 'http://localhost:8000/game-res/';  //�滻Ϊ��Ӧ��Զ�����ص�ַ
var manifest = {
...
while ( i < process.argv.length) {
    ...
    case '-u' :
        ...
        manifest.remoteManifestUrl = url + manifest.version + '/project.manifest';
        manifest.remoteVersionUrl = url + manifest.version + '/version.manifest';
        i += 2;
        break;
    case '--version' :
    case '-v' :
...
function readDir (dir, obj) {
    ...
    for (var i = 0; i < subpaths.length; ++i) {
        ...
        if (stat.isDirectory()) {
            ...
            // Size in Bytes
			if (path.extname(subpath).toLowerCase() !== '.manifest') {
				size = stat['size'];
...
var doWork = function () {
    ...
    var destFileDir = path.join(dest, '' + manifest.version);
